...
def peliculas_duracion(Pelicula: str):
    movie = df[df['title'] == Pelicula].iloc[0]
    duracion = movie['runtime']
    anio = movie['release_year']
    return f"Duración: {duracion}. Año: {anio}"
@app.get("/franquicia/{Franquicia}")
def franquicia(Franquicia: str):
    franquicia_movies = df[df['belongs_to_collection'].str.contains(Franquicia, na=False)]
...
def peliculas_pais(Pais: str):
    count = df[df['production_countries'].str.contains(Pais)].shape[0]
    return f"Se produjeron {count} películas en el país {Pais}"
@app.get("/productoras_exitosas/{Productora}")
def productoras_exitosas(Productora: str):
    productora_movies = df[df['production_companies'].str.contains(Productora)]
...
def get_director(nombre_director: str):
    peliculas_director = df[df['director'].str.contains(nombre_director)]
...
def recomendacion(titulo: str):
    indice_peli = df[df['title'] == titulo].index[0]
    sim_scores = list(enumerate(matriz_simil[indice_peli]))
    sim_scores = sorted(sim_scores, key=lambda x: x[1], reverse=True)
    top_pelis_simil = [df.iloc[sim_score[0]]['title'] for sim_score in sim_scores[1:6]]
    return {
        'titulo ': titulo,
